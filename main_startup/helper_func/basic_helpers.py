...
def cb_wrapper(func):
    async def wrapper(client, cb):
        ...
        if cb.from_user.id not in users:
            await cb.answer(
                "എൻ്റെ സുൽത്താനയായ ജിന്നിനെ തേടി ഈ ജിന്ന് കാത്തിരിക്കുകയാണ് അവൾ വന്നാൽ പിന്നെ ചുറ്റും ള്ളു ഒന്നും കാണാൻ പറ്റില്ല എൻ്റെ ജിന്നേ.",
                cache_time=0,
                show_alert=True,
            )
        else:
                    ...
                    (
                        InlineQueryResultArticle(
                            ...
                            input_message_content=InputTextMessageContent(
                                "**Hai!** Well, I am Not For You, I Only Work For My Master. Why Don't You Deploy Your Own NAZRIYAGUARD ?"
                            ),
                        )
                    )
                ],
